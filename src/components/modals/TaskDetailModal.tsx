...
const TaskDetailModal: FC<TaskDetailModalProps> = ({ isOpen, onClose, taskId }) => {
    ...
    const { data: availableTasks, isLoading: availableTasksLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            try {
                // Get project ID (handle both object and string formats)
                ...
                return allTasks.filter((t: any) => {
                    ...
                    // Don't show tasks that are already dependencies
    ...
    const addWatcherMutation = useMutation({
        ...
        onError: (error: any) => {
            alert(error.response?.data?.error || 'Failed to add watcher');
        },
    });
    ...
    const removeWatcherMutation = useMutation({
        ...
        onError: (error: any) => {
            alert(error.response?.data?.error || 'Failed to remove watcher');
        },
    });
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title={isEditing ? 'Edit Task' : 'Task Details'} size="xl">
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    {task.subtasks && task.subtasks.length > 0 && (
                                        <span className="text-xs text-gray-500">
                                            {task.subtasks.filter((s: any) => s.isCompleted).length} of{' '}
                                    ...
                                    <div className="mb-3">
                                        <div className="w-full bg-gray-200 rounded-full h-2 mb-3">
                                            <div
                                                ...
                                                style={{
                                                    width: `${(task.subtasks.filter((s: any) => s.isCompleted).length /
                                        ...
                                        <div className="space-y-2 mb-3">
                                            {task?.subtasks?.length > 0 ? (
                                                task.subtasks.map((subtask: any) => (
                                                    <div
                                                        key={subtask._id}
                                                        className="flex items-center space-x-3 p-2.5 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors group"
                                                    >
                                                        <input
                                                            type="checkbox"
                            ...
                            <div>
                                ...
                                <div>
                                    ...
                                    <label
                                        ...
                                        className={`flex items-center justify-center w-full px-4 py-3 border-2 border-dashed rounded-lg cursor-pointer transition-colors ${uploadAttachmentMutation.isPending
                                        ...
                                        {task.watchers.map((watcher: any) => (
                                            ...
                                            >
                                                ...
                                                {canEdit && watcher._id !== currentUserId && (
                                                    <button
                                                        onClick={() => removeWatcherMutation.mutate(watcher._id)}
                            ...
                            <div className="p-4 bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-lg">
                                ...
                                {activeTimer?.isRunning && !hasActiveTimerForTask && (
                                    <p className="text-xs text-amber-600">
                                        ⚠️ Timer is running on another task. Stop it first to start a new one.
                                    </p>
                                )}
                            </div>
                        ...
                        <>
                            ...
                            {task.updatedAt && task.updatedAt !== task.createdAt && (
                                <div>
                                    ...
                                    {task.updatedBy && (
                                        <p className="text-xs text-gray-500 mt-0.5">by {task.updatedBy.name}</p>
                                    )}
                                </div>
                            )}
                        </>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-lg p-6 max-w-md w-full">
                        ...
                        >
                            ...
                            {!availableTasksLoading && availableTasksArray.length > 0 ? (
            ...
            {/* ✅ NEW: Add Watcher Modal */}
            {showWatcherModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-lg p-6 max-w-md w-full">
                        <h3 className="text-lg font-semibold mb-4">Add Watcher</h3>
                        <p className="text-sm text-gray-600 mb-4">
                            Select a team member to watch this task and receive updates.
                        </p>
                        <select
                        ...
                        >
                            ...
                            {projectMembersArray
                                .filter(
                                    (member: any) =>
                                        !task.watchers?.some((w: any) => w._id === member._id)
                                )
                                .map((member: any) => (
                                    <option key={member._id} value={member._id}>
